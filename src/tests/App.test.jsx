...
describe('App component', () => {
  ...
  beforeEach(() => {
    originalGetData = FetchData.getData;
  ...
  it('renders correct heading', () => {
    expect(
      screen.getByRole('heading', { name: 'RPG Memory Game' }).textContent,
    ).toMatch(/RPG Memory Game/i);
  });

  it('renders score after card is clicked', async () => {
    const user = userEvent.setup();
    const card = await waitFor(() => screen.getByText('Grand Theft Auto V'));
  ...
  it('modal displays correct message depending on game outcome', async () => {
    const user = userEvent.setup();

    await waitFor(() => screen.getByText('Grand Theft Auto V'));
    for (const item of mockedGameData) {
      user.click(screen.getByText(item.name));
    }
